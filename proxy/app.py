...
def build_record(trace_id, req, resp, sql_comment_added):
    ...
    rec = {
        ...
        "payload": {
            "payload_id": None, "category": None,
            "vector": None, "template_ref": None,
            "raw_payload_redacted": None
        },
        "response": {
...
def proxy(path):
    ...
    # ---- SQL comment trick (parameter modification) ----
    sql_comment = f"/* trace_id={trace_id} */"
    modified_args = {}
    sql_comment_added = False
    for k, v in request.args.items():
        if isinstance(v, str) and v.strip():
            # inject comment at the end of param value (safe, no breakage)
            modified_args[k] = f"{v} {sql_comment}"
    ...
    try:
        resp = requests.request(
            ...
            url=target,
            params=modified_args,  # send modified params
            headers=headers,
    ...
    response_headers = [
        (name, value)
        for (name, value) in (
            resp.raw.headers.items() if resp.raw and resp.raw.headers else []
        )
